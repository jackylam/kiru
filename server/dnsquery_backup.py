...
class DNSQuery:
	...
	Q_TYPES = {1: 'A', 2: 'NS', 5: 'CNAME', 15: 'MX', 28: 'AAAA', 33: 'SRV', 35: 'NAPTR'}
	...
	def __init__(self, thread_id, buf):
		...
		if logger.level == logging.DEBUG:
			logger.debug('\n Thread ID = %d \n domain = %s \n qtype = %d \n qclass = %02x \n tid = %02x \n qr = %02x '
						 '\n opcode = %02x \n aa = %02x \n tc = %02x \n rd = %02x \n zero = %02x \n rcode = %02x \n '
						 'qtype = %02x \n qclass = %02x \n qdcount = %02x \n ancount = %02x \n nscount = %02x'
						 '\n arcount = %02x', thread_id, self.domain, self.qtype, self.qclass, self.tid, self.qr,
	...
	def get_record(self):
		...
		try:
			...
			cursor = conn.cursor()
			query = 'SELECT id, domain_id, name, type, content, ttl, priority, change_date, disabled, order_name, auth, section' \
					' FROM records WHERE name = %s and type = %s and disabled = 0'
			cursor.execute(query, (self.domain, DNSQuery.Q_TYPES[self.qtype]))
			rs = cursor.fetchall()
			...
			logger.error(e)
			self.rcode = DNSQuery.SERVER_FAILURE
